...
def surchTeta(A,eps,teta):
	...
	for v in val_p:
		...
		if(abs(v.real)<0.001) and res.imag<v.imag and v.imag>=0:
			res = v
...
def abscissae(M,eps,axi = None):
	...
	while(abs(point_prec-point_cour)>0.000001 and cpt<100):
		...
		pointempY_list = []
		pointempY_list = surchVect(Matx,A,eps,point_cour.real)
...
def radii(M,eps,axi = None):
	...
	while(abs(point_temp-point_prec)>0.00001 and cpt<100):
		print(cpt)
		point_prec = point_temp
		r,phi = cma.polar(point_temp)
		list_point = intersect_cicle(A,eps,r)
		j = 0
		while(j<len(list_point)):
				...
				j = j+2
		#plt.plot(point_temp.real,point_temp.imag,"b:.")
		cpt = cpt+1
	plt.plot(point_temp.real,point_temp.imag,"b:.",label ="rayon max r1")
