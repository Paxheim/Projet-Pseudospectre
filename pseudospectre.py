...
instruction = "Veuillez rentrer les paramètres du programme\nInsérer les epsilon par ordre croissant en les séparant par un espace"
res =''
b1 = False
...
while(temp==1):
	...
	if (reply==choices[0]):
		if(res != fields[1] and res != fields[2]):
			...
			#cas particulier de la matrice aléatoire
				...
				n=multenterbox(msg=instruction,title="matrice aléatoire",fields=f,values=(10,10,"1 2 3 4 5 6",10000,"non","non","non"))
				...
				if(n_mat ==f_mat[1]):
					n=multenterbox(msg= instruction,title="matrice de Fibonacci",fields=f,values=(15,"0.01 0.05 0.1 0.5 1",10000,"non","non","non"))
					if(n==None):
						break
					m = matrice_fibo(int(n[0]))
				elif(n_mat ==f_mat[2]):
					n=multenterbox(msg= instruction,title="matrice de Toeplitz",fields=f,values=(64,"0.0000001 0.000001 0.00001 0.0001 0.001 0.01 ",10000,"non","non","non"))
					if(n==None):
						break
					m = matrice_top(int(n[0]))
				elif(n_mat ==f_mat[3]):
					n=multenterbox(msg= instruction,title="matrice de Hilbert",fields=f,values=(20,"0.0000001 0.000001 0.00001 0.0001 0.001 0.01 1",10000,"non","non","non"))
					if(n==None):
						break					
					m = hilb(int(n[0]))
				elif(n_mat ==f_mat[4]):
					n=multenterbox(msg= instruction,title="matrice smoke",fields=f,values=(64,"0.0000001 0.000001 0.00001 0.0001 0.001 0.01",10000,"non","non","non"))
					if(n==None):
						break					
					m = smoke(int(n[0]))
				elif(n_mat ==f_mat[5]):
					...
					n=multenterbox(msg= instruction,title="matrice smoke",fields=f,values=(32,"0.0000001 0.000001 0.00001 0.0001 0.001 0.01",10000,"non","oui","non"))
					if(n==None):
						break					
	...
	if(reply == choices[1]):
		...
		if(n_mat == f_mat[0]):
			...
			n=multenterbox(msg= instruction,title="matrice aléatoire",fields=f,values=(10,10,"3",0.001,0.1))
			if(n==None):
				break
			m = matrice_random(int(n[0]),int(n[1]))
			...
			if(n_mat ==f_mat[1]):
				n=multenterbox(msg=instruction,title="matrice de Fibonacci",fields=f,values=(15,"0.05 0.1 0.5",0.001,0.1))
				if(n==None):
					break
				m = matrice_fibo(int(n[0]))
			elif(n_mat ==f_mat[2]):
				n=multenterbox(msg=instruction,title="matrice de Toeplitz",fields=f,values=(64,"0.01 ",0.001,0.1))
				if(n==None):
					break				
				m = matrice_top(int(n[0]))
			elif(n_mat ==f_mat[3]):
				n=multenterbox(msg=instruction,title="matrice de Hilbert",fields=f,values=(20,"1",0.001,0.1))
				if(n==None):
					break				
				m = hilb(int(n[0]))
			elif(n_mat ==f_mat[4]):
				n=multenterbox(msg= instruction,title="matrice smoke",fields=f,values=(32,"0.01",0.001,0.07))
				if(n==None):
					break				
				m = smoke(int(n[0]))
			elif(n_mat ==f_mat[5]):
				n=multenterbox(msg= instruction,title="matrice smoke",fields=f,values=(32,"0.01",0.001,0.05))
				if(n==None):
					break				
				m = matrice_top2(int(n[0]))
			elif(n_mat ==f_mat[6]):
				n=multenterbox(msg=instruction,title="pseudospectre en spirale",fields=f,values=(64,"0.01",0.0001,0.07))
				if(n==None):
					break				
				m = top3(int(n[0]))
			elif(n_mat ==f_mat[7]):
				n=multenterbox(msg= instruction,title="pseudospectre en X",fields=f,values=(32," 0.01",0.0001,0.1))
				if(n==None):
					break				
				m = matrice_top4(int(n[0]))
